...
# Reads and edits a MOLPRO basis set file (.mpro) that the user provides
# and converts it to an ORCA compatible one for further operations.
molpro_filenames = glob.glob('**/*.mpro',recursive=True)
filename_molpro_original = len(molpro_filenames)
if filename_molpro_original == 0:
    print('Basis set file in a MOLPRO format (.mpro) was not found')
    sys.exit()
