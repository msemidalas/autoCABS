...
# Requires MolSSI-BSE (Basis set exchange) version 0.9 and Chemtools version 0.9.2

# Read and edit one MOLPRO (e.g., c.mpro) or ORCA basis (e.g.,c.orca).
# Remove any generated AutoCABS files that might be here from a previous run.
for f in glob.glob("*-AutoCABS*"):
    os.remove(f)

# Locate the MOLPRO or ORCA basis set files
...
filename_orca_original = len(orca_filenames)
...
elif filename_orca_original is not 0:
    print('Basis set file in an ORCA format was found.')
...
elif (filename_molpro_original is 0 and filename_orca_original is 0):
    print('No basis set file was found.')
    sys.exit()
...
df['I_Type_Exp'] = np.where(df['Shell'] == 'I',1,0)
...
I_orbital_exponents = df[df['I_Type_Exp']==1]
...
#I shells
I_orbital_exponents_with_contraction_coeff_1 = I_orbital_exponents[I_orbital_exponents['Contraction']==1]
I_orbital_exponents_with_contraction_coeff_1_np = np.array(I_orbital_exponents_with_contraction_coeff_1['Orbital_Exponent'],dtype="float64")
if I_orbital_exponents_with_contraction_coeff_1_np.size != 0:
    I_orbital_exponents_with_contraction_coeff_1_np = np.vstack(I_orbital_exponents_with_contraction_coeff_1_np)
    I_orbital_exponents_with_contraction_coeff_1_np = unique_rows(I_orbital_exponents_with_contraction_coeff_1_np)
    I_orbital_exponents_with_contraction_coeff_1_np = np.sort(I_orbital_exponents_with_contraction_coeff_1_np, axis=-1)
#Contraction coeff. does not equal to 1
I_orbital_exponents_with_contraction_coeff_not_1 = I_orbital_exponents[I_orbital_exponents['Contraction']!=1]
I_orbital_exponents_with_contraction_coeff_not_1_np = np.array(I_orbital_exponents_with_contraction_coeff_not_1['Orbital_Exponent'],dtype="float64")
if I_orbital_exponents_with_contraction_coeff_not_1_np.size != 0:
    I_orbital_exponents_with_contraction_coeff_not_1_np = np.vstack(I_orbital_exponents_with_contraction_coeff_not_1_np)
    I_orbital_exponents_with_contraction_coeff_not_1_np = unique_rows(I_orbital_exponents_with_contraction_coeff_not_1_np)
    I_orbital_exponents_with_contraction_coeff_not_1_np = np.sort(I_orbital_exponents_with_contraction_coeff_not_1_np, axis=-1)
print('\n I main orbital exponents with contraction coefficient equal to 1:\n',I_orbital_exponents_with_contraction_coeff_1_np)
print('\n I main orbital exponents with contraction coefficient not equal to 1:\n',I_orbital_exponents_with_contraction_coeff_not_1_np)
...
# I exponents
# Find outermost exponent with contraction coefficient not equal to 1
I_orbital_exponents_outermost_decontracted = np.setdiff1d(I_orbital_exponents_with_contraction_coeff_not_1_np.flatten(),
                                                          I_orbital_exponents_with_contraction_coeff_1_np.flatten())
if I_orbital_exponents_outermost_decontracted.size != 0:
    I_orbital_exponents_np = concatenate_and_short(I_orbital_exponents_with_contraction_coeff_1_np.flat,
                                                   I_orbital_exponents_outermost_decontracted[0,].flat,kind='mergesort')
    print('\n I orbital exponents and a single outermost decontracted exponent\n',I_orbital_exponents_np)
else:
    I_orbital_exponents_np = I_orbital_exponents_with_contraction_coeff_1_np
    print('\n I orbital exponents are all with contraction coeff. equal to 1\n',I_orbital_exponents_np)
...
if I_orbital_exponents_np.size == 1:
  I_orbital_exponents_np = 1.5 * H_orbital_exponents_np
  print('\nI orbital exponents were taken as multiples of H-type shells.\n')
#################################################################################
...
if I_orbital_exponents_np.size not in {0,1}:
    geom_mean_I = (I_orbital_exponents_np[:-1] * I_orbital_exponents_np[1:]) ** (1 / 2)
    geom_mean_I = unique_rows(np.vstack(geom_mean_I))
    print('\nGeometric mean of I orbital exponents\n', geom_mean_I)
else:
    geom_mean_I = np.empty(0)
    print('\nGeometric mean of I orbital exponents cannot be calculated from current main orbital exponents:', geom_mean_I)
...
if I_orbital_exponents_np.size not in {0,1}:
    tight_exponent_I_np = ((I_orbital_exponents_np[1:] ** 2) / I_orbital_exponents_np[:-1])
    I_exponents_summary_np = concatenate_and_short(geom_mean_I.flat,tight_exponent_I_np[-1,].flat, kind='mergesort')
    diffuse_exponent_I_np = ((I_orbital_exponents_np[:-1] ** 2) / I_orbital_exponents_np[1:])
    I_exponents_summary_np = concatenate_and_short(I_exponents_summary_np.flat,diffuse_exponent_I_np[0].flat, kind='mergesort')
    print('\n I orbital exponents, tight and a diffuse exponent:\n', I_exponents_summary_np)
else:
    I_exponents_summary_np = geom_mean_I.flatten()
...
while(True):
    ...
    if F_orbital_exponents_np.size == 0 and D_orbital_exponents_np.size not in {0,1}:
        ...
        break
    if G_orbital_exponents_np.size == 0 and F_exponents_summary_np.size not in {0,1}:
    ...
    if H_orbital_exponents_np.size == 0 and G_exponents_summary_np.size not in {0, 1}:
        ...
        break
    if I_orbital_exponents_np.size == 0 and H_exponents_summary_np.size not in {0, 1}:
        I_exponents_summary_np = (H_exponents_summary_np[:-1] * H_exponents_summary_np[1:]) ** (1 / 2)
        I_exponents_summary_np = unique_rows(np.vstack(I_exponents_summary_np)).flatten()
        print('Generated I orbital exponents (extended version)\n', I_exponents_summary_np)
        break
    else:
        break
...
write_exponents_summary_to_gbs_file(I_exponents_summary_np,'I_exponents_summary_np','I')

generated_exponents_summary_gbs = ["S_exponents_summary_np_gbs","P_exponents_summary_np_gbs","D_exponents_summary_np_gbs",
                               "F_exponents_summary_np_gbs","G_exponents_summary_np_gbs","H_exponents_summary_np_gbs","I_exponents_summary_np_gbs"]
...
write_exponents_summary_to_orca_file(I_exponents_summary_np,'I_exponents_summary_np','I')
...
write_exponents_summary_to_tm_file(I_exponents_summary_np,'I_exponents_summary_np','I')

generated_exponents_summary_tm = ["S_exponents_summary_np_tm","P_exponents_summary_np_tm","D_exponents_summary_np_tm",
                               "F_exponents_summary_np_tm","G_exponents_summary_np_tm","H_exponents_summary_np_tm","I_exponents_summary_np_tm"]
...
discardthis = "%s-AutoCABS.gbs" % element_ID
if os.path.exists(f'{discardthis}') == True:
    os.remove(discardthis)
discardthisone = '%s.gbs-formatted_basis' % element_ID
if os.path.exists(f'{discardthisone}') == True:
    os.remove(discardthisone)
discardthisone2 = '%s.orca-formatted_basis' % element_ID
if os.path.exists(f'{discardthisone2}') == True:
    os.remove(discardthisone2)
discardthisone3 = '%s.gbs' % element_ID
if os.path.exists(f'{discardthisone3}') == True:
    os.remove(discardthisone3)
